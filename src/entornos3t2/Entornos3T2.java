...
public class Entornos3T2 {
    ...
    public static void capturarLista(String[] pakemon, Pakemon juego1) {
        int longitud = pakemon.length;//he puesto la longitud en una variable porque sino en cada vuelta del bucle tiene que ir a contar la longitud.
        for (int i = 0; i < longitud; i++) { 
            if (juego1.getPakeballs() > 0) {
                juego1.capturarPakemon(pakemon[i]);
            } else {
                System.out.println("No tienes Pakeballs");
                break;//le he puesto un break para que si no tiene pokeballs el bucle no siga ejecutandose.
            }
        }
    }
